...
export function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-50 hidden md:flex h-full w-64 lg:w-72 flex-col border-r border-zinc-200 bg-white dark:border-zinc-800 dark:bg-[#0a0a0a]">
      ...
      {/* Auth Footer with Points Display */}
        ...
        {isLoading ? (
          ...
          <div className="flex items-center justify-between rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 dark:border-zinc-800 dark:bg-zinc-900/50 shadow-sm">
            <Link to={`/profile/${user.username}`} className="flex items-center gap-3 overflow-hidden hover:opacity-80 transition-opacity">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-zinc-900 dark:bg-zinc-100 shrink-0 shadow-sm">
          ...
          <NavLink to="/login" className="flex items-center justify-center gap-2 rounded-md bg-zinc-900 px-4 py-3 text-sm font-medium text-zinc-50 hover:bg-zinc-900/90 dark:bg-zinc-50 dark:text-zinc-900 dark:hover:bg-zinc-50/90 transition-colors">
